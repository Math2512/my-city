<x-app-layout>
    ...
    <div x-data="{ isOpen: false, groupData: { id: '', name: '' } }">
    <div class="py-8">
        <div
                        ...
                        <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="row"
                                ...
                                <img class="w-10 h-10 rounded-full"
                                    src="{{asset($group->picture->url ?? 'test/4.png' )}}"
                                    alt="Jese image">
                                <div class="ps-3">
                                    <div class="text-base font-semibold">{{$group->name}}</div>
                                    <div class="font-normal text-gray-500">Test</div>
                                </div>
                            </th>
                            <td class="px-6 py-4">
                                {{$group->activity}}
                            ...
                            <td class="px-6 py-4">
                                @foreach ( $group->users as $user )
                                    @can('update', $user)
                                        <a class="text-blue-600 hover:underline dark:text-blue-500" href="{{route('users.edit', $user)}}" >{{ $user->name }}</a>
                                    @else
                                        {{ $user->name }}
                                    @endcan
                                    @if (!$loop->last)
                            ...
                            @if (auth()->user()->is_manager())
                                <td class="px-6 py-4 flex justify-end space-x-2">
                                    ...
                                    @endcan
                                    @can('delete', $group)
                                        <a href="#" x-on:click.prevent="isOpen = true; groupData = {id: '{{$group->id}}', name: '{{$group->name}}'}" data-modal-target="myModal" data-modal-show="myModal" class="text-blue-600 hover:underline dark:text-blue-500">
                    ...
                    <div class="relative w-full max-w-2xl max-h-full">
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div class="p-6 space-y-6">
                                <p class="text-gray-500 dark:text-gray-400">Etes vous sur de vouloir supprimer <span x-text="groupData.name"></span> ?</p>
                                <div class="flex justify-center py-2 px-4">
                                    ...
                                    <form method="POST" :action="'{{ route('groups.destroy', '') }}/' + groupData.id">
                                        @method('DELETE')
                                        @csrf
